...
function readConfig(): DeployConfig {
  ...
  catch { return { method: "vercel", gitRemote: "origin", gitBranch: "main", buildCommand: "npm run build", postBuild: "", note: "" }; }
}
...
export async function POST(req: Request) {
  ...
  if (IS_VERCEL) {
    ...
    try {
      ...
      // 更新 ref
      const patchRes = await fetch(`${GITHUB}/git/refs/heads/main`, {
        method: "PATCH", headers,
        body: JSON.stringify({ sha: newCommitData.sha, force: false }),
      });
      if (!patchRes.ok) {
        const patchErr = await patchRes.json().catch(() => ({ message: "Unknown" }));
        return NextResponse.json({ success: false, message: `触发失败: ${patchErr.message || patchRes.statusText}` }, { status: 500 });
      }
  ...
  try {
    ...
    } else if (config.method === "git") {
      const remote = config.gitRemote || "origin";
      const branch = config.gitBranch || "main";
      try {
        const { stdout } = await execAsync(`git push ${remote} ${branch}`, { cwd, timeout: 120000 });
        return NextResponse.json({ success: true, message: `✅ 已推送到 ${remote}/${branch}`, detail: stdout.slice(-200) });
      } catch (e: any) {
        return NextResponse.json({ success: false, message: "推送失败，请用 Vercel 方式重试", detail: e.message?.slice(0, 300) || "" });
      }
    } else {
      try { await execAsync(config.buildCommand || "npm run build", { cwd }); return NextResponse.json({ success: true, message: "✅ 构建完成" }); }
      catch (e: any) { return NextResponse.json({ success: false, message: "构建失败", detail: e.message?.slice(0, 300) || "" }); }
    }
  } catch (e: any) {
